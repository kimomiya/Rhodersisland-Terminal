...
final _eventItems = Provider.autoDispose((ref) {
  final items = ref.watch(itemsProvider.state).items;
  return items.filter((item) => item.type == ItemType.eventItem);
});

class EventItemsView extends StatelessWidget {
  ...
  Widget _buildGrid() {
    return Consumer(
      builder: (contexxt, watch, child) {
        final eventItems = watch(_eventItems);
